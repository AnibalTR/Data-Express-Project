...
const validate = evt => {
    ...
    if(!email_pattern.test(email.value)){
        error_msg = "email must have at least one character, followed by an '@', then at least two characters, followed by a dot, and then at least two more characters";
        errors = true;
        
    }
    if(!password_pattern.test(password.value)){
        error_msg = "password must be at least 8 characters and include one capitalized letter, one digit, and one special character";
        errors = true;
